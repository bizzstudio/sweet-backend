...
const updateOrderWebHook = async (req, res) => {
  ...
  if (req.body.ResponseCode === 0) {
    try {
      ...
      // כל סגירת ההזמנה (Processing, מלאי, קופון, מבצעים, קופון הטבה, מייל)
      // יושבת ב-lib/orders/finalizeOrder — אותו קוד בדיוק משמש גם את המסלול
      // של חנות ללא תשלום (PAYMENT_DISABLED), כדי ששני המסלולים לא ייפרדו.
