...
const getShowingCategory = async (req, res) => {
  try {
    ...
    // סדר הקטגוריות הרצוי בחנות. הרשימה הקודמת נגררה מפרויקט "תמרים בתומר"
    // ולא הכילה אף אחת מהקטגוריות של המתוקים של בני — התוצאה הייתה שכולן
    // קיבלו 999 והופיעו בסדר אקראי. השמות כאן הם name.he כפי שהוא ב-DB.
    const order = {
      "מזון": 1,
      "פירות": 2,
      'ח.ניקוי+ח"פ': 3,
    ...
    // מיון הקטגוריות לפי הסדר הרצוי.
    // ‎?. הכרחי ולא קוסמטי: name הוא Object חופשי בסכימה, וקטגוריה שנוצרה בלי
    // name.he הייתה מפילה את כל ה-endpoint ב-TypeError — כלומר חנות בלי שום
    // קטגוריה. קטגוריה כזו פשוט נדחקת לסוף.
    const sortedCategories = categories.sort((a, b) => {
      const orderA = order[a.name?.he] || 999; // אם הקטגוריה לא נמצאת באובייקט, היא תידחק לסוף
      const orderB = order[b.name?.he] || 999;
