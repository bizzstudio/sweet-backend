    ...
    const storeSetting = await Setting.updateOne(
      ...
      },
      {
        $set: {
          ...
          // סף התראת מלאי נמוך
          "setting.low_stock_threshold": req.body.setting.low_stock_threshold,

          // משלוח חינם מעל סכום קנייה
          "setting.free_shipping_status": req.body.setting.free_shipping_status,
          "setting.free_shipping_threshold":
            req.body.setting.free_shipping_threshold,
...
const getStoreSeoSetting = async (req, res) => {
  ...
  try {
    ...
    if (path.startsWith('/product/')) {
      ...
      const product = await Product.findOne({ slug: slug }).populate('category', 'name icon coloredIcon');
