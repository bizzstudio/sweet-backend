...
const CustomerBillingSchema = new mongoose.Schema(
  {
    ...
    // כתובת המייל שאליה נשלחות החשבוניות, כשהיא שונה מכתובת הכניסה לחנות.
    //
    // למה שדה נפרד ולא customer.email: השדה ההוא הוא מפתח ההתחברות והוא
    // ייחודי במסד. הנהלת חשבונות אחת שמשרתת כמה לקוחות (סניפים, חברות
    // באותה קבוצה) לא יכולה להופיע בו פעמיים — ניסיון כזה נדחה ב-409.
    // כאן אין ייחודיות, ולכן אותה כתובת יכולה לשמש כמה לקוחות.
    invoiceEmail: { type: String, required: false },

    // המייל שנרשם בכרטיס הלקוח ב-iCount בסנכרון האחרון. iCount שולח את
    // החשבונית לכתובת שרשומה אצלו, ולא לזו שבמסד שלנו — ולכן תיקון מייל
    // אצלנו חייב לדחוף כרטיס מעודכן לפני ההפקה הבאה. ההשוואה לשדה הזה היא
    // מה שמאפשר לזהות את השינוי בלי לפנות ל-iCount בכל מסמך.
    icountSyncedEmail: { type: String, required: false },
    ...
    // אחוז הנחה קבוע שסוכם עם הלקוח, ויורד מכל מסמך שמופק לו — תעודת
    // משלוח, הצעת מחיר והחשבונית החודשית.
    //
    // למה כאן ולא ב-erp.discountPercent: השדה ההוא הוא *מה שהיה כתוב
    // באקסל של מנוע*, והוא נדרס בכל ייבוא חוזר. ההנחה היא החלטה שלנו על
    // אופן החיוב, ואסור שייבוא של קובץ ישן ישנה בשקט כמה הלקוח משלם.
    //
    // ההנחה מחושבת על סכום השורות בלבד (לפני משלוח ולפני מע"מ), ונשמרת
    // על המסמך גם באחוזים וגם בשקלים — האחוז יכול להשתנות מחר, והמסמך
    // חייב להישאר מוסבר.
    //
    // ריק (undefined) = נופלים על erp.discountPercent מהייבוא, כדי
    // ש-21 הלקוחות שההנחה שלהם הגיעה מהאקסל יקבלו אותה בלי הקלדה מחדש.
    // 0 מפורש = בלי הנחה, וגובר על הייבוא.
    discountPercent: { type: Number, required: false, min: 0, max: 100 },

    // האם החשבונית החודשית מרכזת את השורות במקום לפרט כל מוצר.
    //
    // מרכזת = שורה אחת לכל קטגוריה ("ריכוז תעודות משלוח — פירות"), ורשימת
    // מספרי התעודות בגוף המסמך. מפורטת = כל שורות המוצרים מכל התעודות,
    // שאצל לקוח עם 12 תעודות בחודש הן מאות שורות.
    //
    // ברירת המחדל היא ריכוז: זו הצורה שהלקוחה ביקשה (30/08/26), והיא גם
...
const customerSchema = new mongoose.Schema(
  {
    ...
    // המייל המשני — של איש הקשר אצל הלקוח. תצוגה בלבד: המערכת לא שולחת
    // אליו דבר. כל מה שיוצא ללקוח (חשבונית, אישור הזמנה, איפוס סיסמה,
    // דיוור) הולך ל-email, או ל-billing.invoiceEmail כשהוגדרה שם כתובת
    // ייעודית לחשבוניות.
    //
    // למה לא unique: אותו איש קשר משרת לפעמים כמה לקוחות (סניפים, חברות
    // באותה קבוצה), וייחודיות הייתה דוחה את השני ב-409.
    //
    // למה לא בתוך erp: erp נבנה מחדש בכל ייבוא של אקסל ההנהח"ש, וכתובת
    // שהוקלדה כאן ביד הייתה נמחקת ברגע שהקובץ הבא לא מכיל אותה.
    contactEmail: {
      type: String,
      required: false,
      lowercase: true,
