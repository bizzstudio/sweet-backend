...
const previewArchiveOrders = async ({ customer, rows }) => {
  ...
  return {
    ...
    samples: groups.slice(0, 10).map((group) => ({
      ...
      exists: existingKeys.has(`${customer._id}:${group.key}`),
    })),
  };
};
...
const importArchiveOrders = async ({ customer, rows, fileName, importedBy }) => {
  ...
  for (const group of limited) {
    ...
    unmatched.forEach((line) => {
      if (!unmatchedSkus.has(line.sku)) {
        unmatchedSkus.set(line.sku, { sku: line.sku, name: line.name, lines: 0 });
      }
      unmatchedSkus.get(line.sku).lines += 1;
    });

    // מסמך שאף שורה בו אינה בקטלוג. הזמנה ריקה אינה מוסיפה מידע לכרטיס
    // הלקוח — היא רק שורה בסכום 0 שנראית כמו תקלה. המק"טים שלה כן מדווחים
    // למעלה, וזה מה שאפשר לעשות איתו משהו.
    if (!cart.length) {
      empty += 1;
      continue;
    }
  ...
  return {
    ...
    unmatchedSkus: [...unmatchedSkus.values()].slice(0, MAX_SAMPLES),
