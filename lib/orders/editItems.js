 ...
 * נתפסה לחיוב, בוטלה בידי אדם, נערכה ידנית, או תעודה ישנה בלי שדה סטטוס
 * כלל. עריכה במצב כזה משנה את ההזמנה ומשאירה את התעודה מאחור — ולכן היא
 * נחסמת עד לאישור מפורש.
 *
 * ⚠️ הרשימה כאן חייבת להישאר זהה לתנאי היציאה של syncFromOrder. מצב שנוסף
 *    שם ולא כאן משמעותו עריכה שעוברת בשקט ומשאירה תעודה ישנה — כלומר
 *    חיוב על מה שהוזמן במקום על מה שנמסר, בלי שאיש יידע.
 */
const inspectNote = async (orderId) => {
  const note = await DeliveryNote.findOne({ order: orderId, kind: { $ne: "manual" } })
    .select("number billing.status billing.cancelledBySync manuallyEdited")
    .lean();
  ...
  // תעודה שנערכה ידנית — הסנכרון מפסיק לגעת בה במכוון (ראו syncFromOrder).
  // היא עדיין "פתוחה", ולכן בלי הבדיקה הזו העריכה הייתה עוברת בלי אזהרה
  // והתעודה הייתה נשארת מאחור.
  if (note.manuallyEdited && status !== "cancelled") {
    return {
      note,
      locked: true,
      status,
      reason:
        `תעודה ${note.number} נערכה ידנית, ולכן שינוי בהזמנה לא יעדכן אותה. ` +
        `אם התיקון נוגע גם לתעודה — יש לערוך אותה במסך התעודה.`,
    };
  }
...
module.exports = {
  editOrderItems,
  OrderEditError,
  // מיוצא לבדיקות בלבד: הרשימה של "מתי התעודה נעולה" חייבת להישאר זהה
  // לתנאי היציאה של syncFromOrder, וזו הדרך לאמת שהיא באמת זהה
  inspectNoteForTest: inspectNote,
};
