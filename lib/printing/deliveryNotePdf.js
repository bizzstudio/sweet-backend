...
// ‏Chromium נטען פעם אחת ומוחזק חי בין הדפסות. עלייה של דפדפן היא כשנייה
// וכ-100MB, ולפתוח אותו מחדש לכל תעודה זה בזבוז — אבל דפדפן שקרס חייב
// לקום מעצמו, ולכן הבדיקה על connected() בכל קריאה.
...
const loadPuppeteer = () => {
  try {
    ...
    if (err.code === "MODULE_NOT_FOUND") {
      throw new Error(
        "puppeteer אינו מותקן בשרת — תעודות לא יודפסו אוטומטית. " +
          "יש להריץ npm ci בתיקיית sweet-backend. " +
...
<style>
  ...
  * { box-sizing: border-box; }
  body {
    margin: 0;
    font-family: "Noto Sans Hebrew", "Noto Sans", "DejaVu Sans", "Arial Hebrew", Arial, sans-serif;
    color: #111827;
    background: #fff;
    font-size: 12px;
  }
