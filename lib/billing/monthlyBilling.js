...
const { loadSummaryLines } = require("./summaryLines");
 ...
 */
const summarizeItems = (items, summaryLines = null) => {
  ...
  const active = [...buckets.values()].filter((b) => money(b.total) > 0);

  // קטגוריה שיש בה גם שורות חייבות וגם פטורות באותה חשבונית.
  //
  // בקטלוג יש שלוש כאלה (מזון, פירות, כללית), ובלי הסימון היו יוצאות שתי
  // שורות בשם זהה ובסכומים שונים — מה שנראה כמו כפילות או כמו טעות.
  // הסימון מופיע *רק* במקרה הזה: בחשבונית רגילה השם הוא בדיוק מה
  // שהתבקש — "ריכוז תעודות משלוח פירות".
  const mixedCategories = new Set(
    active
      .filter((b) => active.some((o) => o.category === b.category && o.vatFree !== b.vatFree))
      .map((b) => b.category)
  );

  const rows = active.map((b) => {
    // מוצר הריכוז מהקטלוג (ראה lib/billing/summaryLines). הוא זה שמעניק
    // לשורה ברקוד — העמודה שרואה החשבון מצליב מולה, ושבחשבוניות
    // הקודמות של הלקוח אף פעם לא הייתה ריקה.
    const product = summaryLines?.for(b.category, b.vatFree) || null;

    const suffix = mixedCategories.has(b.category)
      ? b.vatFree
        ? ' (פטור ממע"מ)'
        : ' (חייב במע"מ)'
      : "";
    ...
    return {
      name: `ריכוז תעודות משלוח ${b.category}${suffix}`,
      quantity: 1,
...
const billClaimedNotes = async ({ customer, notes, claimToken, label, month, emailDocument, immediate = false, partial = false }) => {
  ...
  // נטען פעם אחת לכל החשבוניות של הלקוח, ולא לכל קבוצה בנפרד
  const summaryLines = summarize ? await loadSummaryLines() : null;
  const invoices = [];
  ...
  for (const group of groups) {
    try {
      ...
      const lines = [...(summarize ? summarizeItems(group.items, summaryLines) : group.items)];
...
const closeCustomerMonth = async (
  ...
  if (dryRun) {
    ...
    const previewLines = summarizeLines ? await loadSummaryLines() : null;
    return {
      customerId: customer._id,
      customerName: label,
      splitByCategory,
      ...
      invoices: groups.map((g) => ({
        ...
        itemCount: summarizeLines ? summarizeItems(g.items, previewLines).length : g.items.length,
...
const creditInvoice = async ({ icountDocNum, reason, reopenNotes = true }) => {
  ...
  const items = [
    ...(summarize ? summarizeItems(rawItems, await loadSummaryLines()) : rawItems),
  ];
