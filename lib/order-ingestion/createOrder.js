...
const buildSystemNote = ({ channel, items = [], deliveryInfo }) => {
  ...
  // שורה עמומה שהמערכת בחרה עבורה מוצר (ראה autoPick ב-resolvers). מאותו
  // נימוק של הכמות המונחת: זו הכרעה של המנוע ולא בקשה של הלקוח, ולכן היא
  // חייבת להיות מול עיני מי שמלקט — הוא היחיד שיכול לתפוס בחירה שגויה לפני
  // שהיא יוצאת מהדלת. הפורמט הוא "מה שהלקוח כתב ← מה שנבחר", כי בלי שני
  // הצדדים אי אפשר לדעת אם הבחירה נכונה.
  const autoPickedNames = items
    .filter((i) => i.autoPicked)
    .map((i) => `${i.rawName} ← ${i.product?.title?.he || i.productTitle}`);
  if (autoPickedNames.length) {
    noteParts.push(`נבחר אוטומטית מבין מוצרים דומים — ${autoPickedNames.join(", ")}`);
  }
...
module.exports = {
  ...
  createErrorOrder,
  buildCartItem,
  // ההקצאה האטומית של מספר ההזמנה + הניסיון החוזר. מיוצא כדי שכל מסלול
  // יצירה אחר (למשל "הזמנה חוזרת" — lib/orders/duplicateOrder) ישתמש באותו
  // מנגנון בדיוק, ולא ייצר לעצמו הקצאה מקבילה.
  saveOrder,
  fetchEligibleOffers,
  calculateShipping,
  checkStock,
