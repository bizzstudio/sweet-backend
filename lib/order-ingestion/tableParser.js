  ...
  rows.forEach((row) => {
    ...
    if (quantity === null) {
      if (name) {
        ...
        const suspectedItem =
          quantityCellExists && !isHeaderRow && !looksLikeLabelledField && name.length >= 4;

        skipped.push({
          raw: row.raw,
          reason: suspectedItem
            ? "שורה עם שם מוצר אך בלי כמות קריאה — ייתכן שפריט לא נקלט"
 ...
 */
const parseOrderText = ({ text, sender = {}, subject } = {}) => {
  ...
  if (!result) {
    ...
    // דטרמיניסטי אין: "לא מצאתי שורות עם כמות" יכול להיות גם שאלה תמימה
    // וגם **הזמנה אמיתית בפורמט שאינו מוכר לי**.
    //
    // לסמן פורמט חדש כ-"לא הזמנה" פירושו לזרוק אותו לפח שאף אחד לא בודק —
    // כלומר לאבד הזמנות בשקט. לכן רק הודעה שנראית שיחתית בבירור מסומנת ככזו,
    // וכל השאר עוברת לטיפול אנושי.
    const looksConversational =
      text.length < 400 && // הודעה קצרה
      /[?？]/.test(text) && // עם סימן שאלה
      !/\d/.test(text.replace(/\d{1,4}[-.\/]\d{1,2}/g, "")); // ובלי מספרים (מלבד תאריכים)

    return {
      isOrder: false,
      notAnOrderReason: looksConversational
        ? "ההודעה נראית כשאלה או בירור, ללא פריטים וכמויות"
        : "לא זוהו שורות פריט עם כמות — ייתכן שזה פורמט הזמנה שאינו מוכר למערכת",
      // כשזה לא חד-משמעי, ההודעה חייבת להגיע לעיני אדם ולא להיקבר
      certainNotOrder: looksConversational,
      customer: emptyCustomer(sender),
      delivery: emptyDelivery(),
      items: [],
      note: null,
