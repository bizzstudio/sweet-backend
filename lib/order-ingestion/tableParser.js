...
const BOILERPLATE_PATTERNS = [
  /^(?:בוקר טוב|צהריים טובים|ערב טוב|שלום|היי|הי|אהלן|תודה רבה|תודה|בברכה|בהצלחה|כל טוב|להתראות|חג שמח|שבת שלום|שבוע טוב)(?:$|[^֐-׿\w])/i,
  ...
  // ── מילות מילוי בשיחה ──
  //
  // ווצאפ אינו טופס. לקוח נזכר באמצע, מהסס, מאשר לעצמו: "אה ורגע", "כן רגע",
  // "אוקי". נמדד בהזמנה אמיתית — שתי שורות כאלה נקראו כשמות מוצר, קיבלו
  // ביטחון נמוך, והביטחון הכולל של ההזמנה הוא ה**מינימום** מבין הפריטים:
  // ארבעה פריטים תקינים נפלו ל"לא הובנה במלואה" בגלל שתי מילות מילוי.
  //
  // ההתאמה היא לשורה **שלמה** ומורכבת ממילות מילוי בלבד. זה מה שמונע פגיעה
  // בשורה אמיתית: "לא תה ירוק" מכילה "לא" אבל גם "תה" ו-"ירוק", ולכן אינה
  // נתפסת. שורה שכולה "לא" או "רגע" אינה שם מוצר בשום קטלוג.
  /^(?:ו?(?:אה+|כן|לא|רגע|אוקי+|okay|ok|סבבה|בסדר|מעולה|יופי|אחלה|בטח|נו|טוב)[\s,.!?]*)+$/i,
  /^-{2,}$/,
  /^_{2,}$/,
  /^https?:\/\//i,
...
const splitRows = (text) =>
  String(text || "")
    ...
    // ── trimEnd ולא ‏/\s+$/ ──
    //
    // שני הביטויים מסירים בדיוק את אותם תווים (‏\s ברגקס של JS הוא בדיוק
    // קבוצת WhiteSpace ∪ LineTerminator ש-trim מסיר), אבל `/\s+$/` על מחרוזת
    // שכולה רווחים הוא ריבועי: המנוע מנסה להתחיל את `\s+` בכל מיקום, וכל
    // ניסיון סורק עד הסוף.
    //
    // נמדד על שורה של 40,000 רווחים: **1,525ms** מול 0.009ms ל-trimEnd —
    // פי 170,000. הקלט אינו מהימן, ולכן זו חסימה של הצינור כולו: הודעת ווצאפ
    // אחת תפסה את השרת לשניות.
...
// ── "3. עוגיות" — הנקודה שבלעה את הכמות ──
//
// הדפוס הזה הוא גם מספור של רשימה ("1. חלב / 2. עוגיות") וגם כמות שנכתבה עם
// נקודה ("3. עוגיות" = שלוש עוגיות), ואי אפשר להכריע ביניהם משורה בודדת.
// ‏enumeratedRows מכריע מהמסמך כולו: רק רצף שלם שמתחיל ב-1 הוא מספור, וכל
// מקרה אחר הוא כמות. הסימון מוסר לפני LINE_PATTERNS, ולכן שורה ממוספרת אמיתית
// לעולם אינה מגיעה לדפוס הכמות שלמטה.
//
// אחרי הסימון בא רווח, או תו שאינו ספרה. שתי הצורות נחוצות: "1. 2 עוגיות"
// היא שורה ממוספרת שיש בה כמות, ו-"3.עוגיות" נכתבת בלי רווח בכלל. מה שנשאר
// בחוץ הוא בדיוק מספר עשרוני — ב-"1.5 ליטר" אחרי הנקודה באה ספרה צמודה.
const ENUM_PREFIX = /^[-*•·]?\s*(\d{1,3})\s*[.)](?:\s+|(?=[^\d\s]))/;

/**
 * אילו שורות הן פריט ברשימה ממוספרת — כלומר המספר בהן הוא סידורי ולא כמות.
 * @returns {Set<number>} אינדקסים של שורות שמהן יש להסיר את הסימון
 */
const enumeratedRows = (rows) => {
  const marked = new Set();

  const numbered = [];
  rows.forEach((row, index) => {
    const match = row.raw.trim().match(ENUM_PREFIX);
    if (match) numbered.push({ index, value: Number(match[1]) });
  });

  // שורה בודדת אינה רשימה. היא גם לא צריכה להיות: "1. חלב" יוצא אותו דבר
  // משני הפירושים, ו-"3. עוגיות" — המקרה שהתחיל את כל זה — הוא כמות.
  if (numbered.length < 2) return marked;
  // מספור מתחיל ב-1 ועולה באחד. "1 טסטר צוייס / 3. עוגיות" אינו רצף כזה,
  // ולכן ה-3 שם הוא כמות ולא מקום ברשימה.
  if (numbered[0].value !== 1) return marked;
  for (let i = 1; i < numbered.length; i += 1) {
    if (numbered[i].value !== numbered[i - 1].value + 1) return marked;
  }

  numbered.forEach(({ index }) => marked.add(index));
  return marked;
};

// שורת רשימה חופשית: "2 מגשי תמרים", "- 3 שקיות פיסטוק", "מגש תמרים x2", "תמרים - 4"
// ── ו' החיבור שדבוקה לכמות ──
//
// בווצאפ מונים פריטים כמו שמדברים: "5 קלסר / ו3 אגסים". ה-ו' דבוקה לספרה
// ולכן השורה אינה מתחילה בספרה, אף דפוס כמות אינו נתפס, והיא נופלת למסלול
// "שורה בלי כמות = יחידה אחת" — כלומר **3 אגסים הופכים ל-1**, והשם שנשלח
// לחיפוש הוא "ו3 אגסים" עם "ו3" כמילה שמרעילה את השאילתה.
//
// נמדד בהזמנה אמיתית מלקוח. ‏ו' אינה נכנסת למחלקת התווים של התבליטים: הם
// סימני פיסוק והיא אות, והכנסתה לשם הייתה מוחקת אות פותחת משמות מוצרים.
//
// ── ‏(?:ו\s*)? ולא ‏ו?\s* ──
//
// הניסוח השני נראה זהה ומכניס backtracking **ריבועי**: שני `\s*` משני צדי
// אופציונלי מייצרים n דרכים לחלק אותו רצף רווחים, וכולן נבדקות לפני הכישלון.
// נמדד על הרגקס הזה: 5,000 רווחים = 27ms, 20,000 רווחים = **407ms**. הקלט
// כאן אינו מהימן — הודעת ווצאפ מותרת עד 40,000 תווים — ולכן זהו וקטור חסימה
// של הצינור, לא אי-יעילות. הניסוח כאן לינארי (0.06ms על 20,000).
const LINE_PATTERNS = [
  { re: /^[-*•·]?\s*(?:ו\s*)?(\d+(?:[.,]\d+)?)\s*[xX*]?\s+(.{2,})$/, qtyFirst: true },
  { re: /^[-*•·]?\s*(.{2,}?)\s*[xX*]\s*(\d+(?:[.,]\d+)?)$/, qtyFirst: false },
  // ‏= הוא מפריד נפוץ בדיוק כמו המקף, והיעדרו עלה הזמנה שלמה: אמן מחשבים
  // כתבה "קולה דיאט = 12 / סודה = 24 / קולה קלאסיק = 24", ואף שורה לא נקראה
  // ככמות. גרוע מהשתיקה — "קולה זירו = 12" כן נכנס להזמנה, אבל כפריט אחד,
  // כי המסלול "שורה בלי כמות = יחידה אחת" קלט אותו עם ה-"= 12" בתוך השם.
  ...
  // ── "3. עוגיות" ──
  //
  // אחרון בכוונה: קודם ינסו את כל הצורות המפורשות. הסייג `[^\d\s]` הוא מה
  // שמונע קריאה של מספר עשרוני כמספור — ב-"1.5 ליטר קולה" אחרי הנקודה בא
  // ספרה, הדפוס אינו נתפס, והשורה נשארת לדפוס הראשון שקורא 1.5 ככמות.
  { re: /^[-*•·]?\s*ו?(\d+)\s*[.)]\s*([^\d\s].+)$/, qtyFirst: true },
];
...
const parseLines = (rows, { channel } = {}) => {
  ...
  // מחושב על המסמך כולו לפני הפירוק — ראה enumeratedRows
  const listMarkers = enumeratedRows(rows);

  rows.forEach((row, index) => {
    // ‏sourceLine הוא תמיד השורה כפי שהלקוח כתב אותה, גם כשהסימון מוסר: זה
    // המפתח שבו extractOrderNote מחריגה שורת פריט מהערת ההזמנה, ושורה שעברה
    // עיבוד לא הייתה נמצאת שם — כלומר הפריט היה חוזר ומופיע גם בהערה למלקט.
    const sourceLine = row.raw.trim();
    // ── כיווץ רצפי רווחים לפני התאמת הדפוסים ──
    //
    // שלושה מדפוסי השורה בנויים כ-`(.{2,}?)\s*[מפריד]`, וצירוף של כמת עצל
    // עם `\s*` הוא backtracking **ריבועי** על רצף רווחים ארוך: לכל אורך
    // אפשרי של הקבוצה העצלה נבדקות כל החלוקות של הרווחים.
    //
    // נמדד על הפרסר עצמו: שורה של 40,000 רווחים = **1.7 שניות**, ועם מילה
    // לפניה 8.7 שניות. הקלט אינו מהימן (הודעת ווצאפ מותרת עד 40,000 תווים),
    // ולכן זו חסימה של הצינור כולו ולא אי-יעילות.
    //
    // הכיווץ לינארי ואינו משנה התאמה: כל הדפוסים משתמשים ב-`\s*`/`\s+`
    // וממילא רואים רצף רווחים כרווח אחד. ‏sourceLine **אינו** נוגע —
    // ‏extractOrderNote מחריגה שורת פריט מההערה לפיו, וכיווץ שלו היה מחזיר
    // את השורה גם כפריט וגם כהערה למלקט.
    const collapsed = sourceLine.replace(/\s+/g, " ");
    const line = listMarkers.has(index)
      ? collapsed.replace(ENUM_PREFIX, "").trim()
      : collapsed;
    ...
    if (isBoilerplate(line)) {
      skipped.push({ raw: sourceLine, reason: "טקסט פתיחה/סגירה" });
      return;
    }
    ...
    if (isContactLine(line)) {
      skipped.push({ raw: sourceLine, reason: "שורת חתימה / פרטי קשר" });
      return;
    }
