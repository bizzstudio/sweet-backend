 ...
 *
 * @returns {Promise<{items: Array, unmatched: Array}>}
 */
const resolveItems = async (llmItems, { contextText = "" } = {}) => {
  const items = [];
  const unmatched = [];
  ...
  for (const raw of llmItems) {
    ...
    const base = { rawName, quantity, unit, note };
    ...
    try {
      ...
      unmatched.push({ ...base, confidence: 0, failReason: `שגיאת התאמה: ${err.message}` });
      continue;
    }
    ...
    if (!match) {
      unmatched.push({
        ...base,
        confidence: 0,
        failReason: "לא נמצא מוצר מתאים בקטלוג",
      });
      continue;
    }
    ...
    if (
      !skuConfirmed &&
      (qualifiers.negations.length || qualifiers.sizes.length || qualifiers.variants.length)
    ) {
      const candidatePool = [
      ...
      const { kept, rejected, appliedFilters, sizeMismatch } = applyQualifiers(
        candidatePool,
        qualifiers
      );
      ...
      if (!kept.length) {
        ...
        unmatched.push({
          ...
          failReason:
            rejected[0]?.reason || "כל המועמדים נפסלו לפי המזהים שהלקוח ציין",
        });
        continue;
      }
    ...
    const unavailableReason = productUnavailableReason(chosenProduct, quantity);
    ...
    if (confidence < AUTO_ACCEPT_CONFIDENCE && !USE_EXTERNAL_AI()) {
      if (confidence >= MIN_ITEM_CONFIDENCE) {
        ...
        unmatched.push({
          ...
          alternatives: match.alternatives,
          productTitle: chosenProduct.title?.he,
          failReason: `זוהו כמה מוצרים אפשריים ואי אפשר להכריע אוטומטית (ביטחון ${confidence}). המועמד המוביל: "${chosenProduct.title?.he}"`,
        });
      }
      continue;
    }
      ...
      try {
        ...
        if (choice.chosenIndex === -1) {
          unmatched.push({
            ...
            failReason: `לא ניתן להכריע איזה מוצר התכוון הלקוח${choice.reason ? ` (${choice.reason})` : ""}`,
          });
          continue;
        }
        ...
        if (!chosenProduct) {
          unmatched.push({
            ...base,
            confidence: 0,
            failReason: "המוצר שנבחר לא נמצא ב-DB",
          });
          continue;
        }
        ...
        unmatched.push({
          ...
          matchScore: match.score,
          alternatives: match.alternatives,
          failReason: `כשל בהכרעה בין מועמדים: ${err.message}`,
        });
        continue;
      }
    }
    ...
    if (confidence < MIN_ITEM_CONFIDENCE) {
      unmatched.push({
        ...
        alternatives: match.alternatives,
        productTitle: chosenProduct.title?.he,
        failReason: `ביטחון זיהוי נמוך מדי (${confidence})`,
      });
      continue;
    }
