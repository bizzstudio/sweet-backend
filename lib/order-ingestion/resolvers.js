...
const { pickFromHistory } = require("../../utils/purchaseHistoryRanking");
...
const resolveItems = async (
  ...
  for (const raw of llmItems) {
    ...
    if (
      ...
      // מי ששרד את הסינון — זו הבריכה שההיסטוריה מדרגת מחדש (ראה למטה)
      survivingPool = kept.map((k) => ({ product: k.product }));
    }
    ...
    // לכתוב.
    //
    // ── ולמה שורה בלי כמות מוחרגת ──
    //
    // ההיסטוריה עונה על "לאיזה מוצר הלקוח התכוון", ולא על "האם השורה הזו היא
    // מוצר בכלל". שורה בלי כמות היא ניחוש של המערכת שמדובר בפריט (ראה
    // droppedAsText למטה), ולכן הנחת המוצריות שלה עדיין לא הוכחה.
    //
    // בלי ההחרגה: "קומה 3 דירה 12" מקבל 0.6 מול מוצר כלשהו בקטלוג, וכל שההיסטוריה
    // צריכה כדי להעלות אותו ל-0.9 היא שהמוצר האקראי הזה יהיה כזה שהלקוח קונה
    // בקביעות — ואז שורת כתובת נכנסת להזמנה כפריט. הכשל אינו סימטרי: שורה
    // אמיתית שנשמטת מופיעה בהערת ההזמנה ובמסך הקליטה, ואילו פריט שנוסף בשקט
    // יוצא במשלוח בלי שאיש ביקש אותו.
    ...
    if (historyPick?.tier === "decisive" && !requestedSizeMissing) {
