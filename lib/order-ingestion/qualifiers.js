...
// ── האם בשורה יש בכלל מילה שמתארת מוצר ──
//
// "5 יח'" אינה שם מוצר: כל מה שכתוב בה הוא כמות ויחידת מידה. ההבחנה נדרשת
// למי שמכריע אם שורה היא טקסט או פריט, וההפרש שם אינו תיאורטי: בקטלוג יש
// מאות כותרות שנושאות את המילה "יח", ולכן *כל אחת מהן* מכילה את כל מילות
// השורה הזו — כלומר כל מבחן כיסוי מילים מאשר אותה. נמדד: ארבע שורות "N יח'"
// בהזמנה אחת הפכו לפריטים שלא זוהו, והעבירו הזמנה שלמה לטיפול ידני.
//
// גרשיים ואפוסטרוף מוסרים לפני ההשוואה, כדי ש-"יח'" יימצא לצד "יח" ו-‏"ק\"ג"
// לצד "קג" — שתי הצורות רשומות ב-UNIT_LOOKUP, וההסרה סוגרת את מה שביניהן.
const hasProductWord = (text) => {
  ...
  return words.some((word) => {
    ...
    return !UNIT_LOOKUP.has(word.toLowerCase()) && !UNIT_LOOKUP.has(clean);
  });
};
 ...
 * @returns {{kept: Array, keptBeforeSilentDrops: Array, rejected: Array, appliedFilters: Array, sizeMismatch: boolean, sizeMismatchLabel: string|null}}
 *          ‏keptBeforeSilentDrops — מי ששרד את מה שהלקוח **אמר**, לפני פסילת
 *          מפרט שלא התבקש. ראה ההסבר בגוף הפונקציה.
 */
const applyQualifiers = (candidates, qualifiers) => {
  ...
  // ── נקודת החיתוך: כאן נגמר "מה שהלקוח אמר" ומתחיל "מה שהוא לא אמר" ──
  //
  // כל מה שמעל נשען על **הוראה מפורשת** של הלקוח: שלילה, מידה שביקש, גרסה
  // שציין. מה שמתחת נשען על שתיקתו.
  //
  // ‏keptBeforeSilentDrops מוחזר כדי שצרכן שיש לו ראיה טובה יותר משתיקה יוכל
  // לעבוד מולו. היום הצרכן היחיד הוא היסטוריית הרכישות, והיא **מדידה**: אצל
  // לקוח אמיתי, "כפיות" הצטמצם מ-21 מועמדים ל-6 והכפיות שהוא קונה 4 פעמים
  // לא היו ביניהן; "נייר טואלט" מ-27 ל-8, "כוסות" מ-91 ל-2 — בכל אחד מהם
  // הכלל מחק בדיוק את המוצר שההיסטוריה יודעת לזהות.
  //
  // הוא **אינו** עוקף שלילה או מידה מבוקשת: אלה סוננו כבר למעלה, ומי שנפסל
  // בהן אינו נמצא כאן.
  const keptBeforeSilentDrops = kept;
